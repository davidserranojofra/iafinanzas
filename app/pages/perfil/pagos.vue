...
<template>
  <div class="flex flex-col min-h-screen bg-dracula-bg pb-10">
    <div class="flex items-center gap-3 px-4 pt-12 pb-6">
      ...
      <h1 class="text-lg font-bold text-dracula-text">Métodos de pago</h1>
    </div>
    ...
    <div class="flex flex-col gap-4 px-4">
      ...
      <template v-if="pending">
        <div v-for="i in 5" :key="i" class="h-16 rounded-2xl bg-dracula-card2 animate-pulse" />
      </template>
      ...
      <template v-else>
        <div
          ...
          :key="metodo"
          class="flex items-center gap-4 bg-dracula-card2 rounded-2xl px-4 py-4 min-h-[64px] cursor-pointer transition-colors active:bg-dracula-card"
          @click="toggle(metodo)"
        >
        ...
        <button
          class="w-full py-4 rounded-2xl text-sm font-semibold text-white transition-opacity"

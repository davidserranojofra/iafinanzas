  ...
  <div class="flex flex-col min-h-screen bg-dracula-bg pb-10">
    ...
    <div class="flex items-center gap-3 px-4 pt-12 pb-6">
      ...
      <h1 class="text-lg font-bold text-dracula-text flex items-center gap-2">
        Notificaciones
        <span v-if="guardando" class="text-[10px] bg-dracula-purple/20 text-dracula-purple font-semibold px-2 py-0.5 rounded-md animate-pulse">Guardando...</span>
      </h1>
    </div>
    ...
    <div v-else class="flex flex-col gap-4 px-4">
      <!-- Error Banner -->
      <div v-if="errorMsg" class="bg-dracula-red/10 border border-dracula-red/25 text-dracula-red p-4 rounded-3xl text-xs flex items-start gap-3">
        <span class="text-sm mt-0.5">⚠️</span>
        <p class="font-medium flex-1">{{ errorMsg }}</p>
      </div>

      <!-- iOS Standalone Alert -->
      <div v-if="esIOS && !esStandalone" class="bg-dracula-orange/10 border border-dracula-orange/25 text-dracula-orange p-4 rounded-3xl text-xs flex items-start gap-3">
        <span class="text-sm mt-0.5">💡</span>
        <div>
          <p class="font-semibold">Instalación requerida en iOS</p>
          <p class="opacity-80 mt-0.5">Para recibir alertas en tu iPhone/iPad, tenés que agregar esta app a inicio ("Compartir" -> "Agregar a pantalla de inicio") y abrirla desde ahí.</p>
        </div>
      </div>

      <!-- Activar Notificaciones -->
      <div class="bg-dracula-card2 rounded-3xl p-4 border border-dracula-muted/10 flex items-center justify-between gap-4 cursor-pointer" @click="toggleNotificaciones">
        <div class="flex items-center gap-3">
          <span class="text-lg">🔔</span>
          <div>
            <p class="text-sm font-semibold text-dracula-text">Alertas de resumen de gastos</p>
            <p class="text-xs text-dracula-muted mt-0.5">Recibí informes periódicos sobre tus consumos</p>
          </div>
        </div>
        <div
      ...
      <Transition name="fade-slide">
        <div v-if="habilitarNotificaciones" class="flex flex-col gap-4">
          ...
          <div class="bg-dracula-card2 rounded-3xl p-4 border border-dracula-muted/10">
            <p class="text-sm font-semibold text-dracula-text mb-1">¿Cada cuántos días querés el resumen?</p>
            <p class="text-xs text-dracula-muted mb-4">La alerta recopilará las compras realizadas en este intervalo</p>
            
            <div class="flex flex-wrap gap-2">
              <button
                v-for="p in presetsIntervalo"
                :key="p.value"
                type="button"
                class="px-3 py-2 rounded-xl text-xs font-semibold border transition-all cursor-pointer"
                :class="intervaloDias === p.value
          ...
          <div class="bg-dracula-card2 rounded-3xl p-4 border border-dracula-muted/10">
            ...
            <div class="grid grid-cols-2 gap-3">
              <div
                class="flex flex-col gap-1.5 p-3 rounded-2xl border cursor-pointer transition-all"
                :class="tipoResumen === 'total'
                  ? 'bg-dracula-purple/10 border-dracula-purple text-dracula-text'
                  : 'bg-dracula-card/30 border-transparent text-dracula-muted hover:bg-dracula-card/50'"
                @click="tipoResumen = 'total'"
              >
                <div class="flex items-center justify-between">
                  <p class="text-xs font-semibold" :class="tipoResumen === 'total' ? 'text-dracula-purple' : 'text-dracula-text'">Solo Total</p>
                  <div class="w-3.5 h-3.5 rounded-full border flex items-center justify-center" :class="tipoResumen === 'total' ? 'border-dracula-purple bg-dracula-purple' : 'border-dracula-muted'">
                    <div v-if="tipoResumen === 'total'" class="w-1 h-1 rounded-full bg-dracula-bg" />
                  </div>
                </div>
                <p class="text-[10px] opacity-75 leading-relaxed">Recibí un aviso corto únicamente con el importe acumulado global.</p>
              </div>

              <div
                class="flex flex-col gap-1.5 p-3 rounded-2xl border cursor-pointer transition-all"
                :class="tipoResumen === 'categorias'
                  ? 'bg-dracula-purple/10 border-dracula-purple text-dracula-text'
                  : 'bg-dracula-card/30 border-transparent text-dracula-muted hover:bg-dracula-card/50'"
                @click="tipoResumen = 'categorias'"
              >
                <div class="flex items-center justify-between">
                  <p class="text-xs font-semibold" :class="tipoResumen === 'categorias' ? 'text-dracula-purple' : 'text-dracula-text'">Por Categorías</p>
                  <div class="w-3.5 h-3.5 rounded-full border flex items-center justify-center" :class="tipoResumen === 'categorias' ? 'border-dracula-purple bg-dracula-purple' : 'border-dracula-muted'">
                    <div v-if="tipoResumen === 'categorias'" class="w-1 h-1 rounded-full bg-dracula-bg" />
                  </div>
                </div>
                <p class="text-[10px] opacity-75 leading-relaxed">Desglosá el resumen para ver importes según tus consumos de interés.</p>
              </div>
            </div>
          </div>
          ...
          <Transition name="fade-slide">
            <div v-if="tipoResumen === 'categorias'" class="bg-dracula-card2 rounded-3xl p-4 border border-dracula-muted/10">
              <p class="text-sm font-semibold text-dracula-text mb-1">Categorías activas en el resumen</p>
              <p class="text-xs text-dracula-muted mb-4">Seleccioná cuáles querés auditar periódicamente</p>
      ...
      <p class="text-center text-xs text-dracula-muted/50 px-4 mt-2">
